...
export default async function PartnerAnalyticsPage() {
  ...
  const funnel = [
    { label: 'Dilamar', value: statusGroup.find((s) => s.status === 'APPLIED')?._count._all ?? 0 },
    { label: 'Ditinjau', value: statusGroup.find((s) => s.status === 'REVIEWED')?._count._all ?? 0 },
    { label: 'Shortlist', value: statusGroup.find((s) => s.status === 'SHORTLISTED')?._count._all ?? 0 },
    { label: 'Wawancara', value: statusGroup.find((s) => s.status === 'INTERVIEW')?._count._all ?? 0 },
    { label: 'Penawaran', value: statusGroup.find((s) => s.status === 'OFFERED')?._count._all ?? 0 },
    { label: 'Diterima', value: statusGroup.find((s) => s.status === 'HIRED')?._count._all ?? 0 },
  ]
  ...
  return (
    <div className="p-6 space-y-8">
      <header>
        <h1 className="font-heading text-2xl md:text-3xl">Analitik Rekrutmen</h1>
        <p className="text-muted-foreground mt-1">Performa rekrutmen 6 bulan terakhir.</p>
      </header>

      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <KPICard label="Total Tampilan" value={totalViews} />
        <KPICard label="Rata-rata Tampilan/Lowongan" value={avgViews} />
        <KPICard label="Total Lamaran (6 bln)" value={applications.length} />
        <KPICard label="Total Lowongan Baru" value={jobs.length} />
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <LineChartCard title="Lamaran per Bulan" data={appliedSeries} />
        <LineChartCard title="Lowongan Baru per Bulan" data={jobsSeries} />
      </div>

      <section>
        <h2 className="font-heading text-xl mb-4">Corong Konversi</h2>
