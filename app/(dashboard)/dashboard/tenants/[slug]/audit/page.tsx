...
import { listTenantAudit } from '@/lib/tenants/audit-queries'

export const metadata = { title: 'Audit Tenant — Dasbor' }
...
export default async function TenantAuditPage({
  ...
  const session = await requireAuth(`/dashboard/tenants/${params.slug}/audit`)
  ...
  const actorEmail = typeof searchParams.actor === 'string' ? searchParams.actor : undefined
  ...
  const validAction =
    action && ACTIONS.includes(action as AuditAction) ? (action as AuditAction) : undefined
  ...
  const { items, total } = await listTenantAudit(
    tenant.id,
    { action: validAction, resource, actorEmail, from: fromDate, to: toDate },
  ...
  const baseHref = `/dashboard/tenants/${tenant.slug}/audit`
  const exportHref = buildHref(`/api/tenants/${tenant.slug}/audit/export`, sp, {
    page: undefined,
  })
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl">
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-heading text-2xl md:text-3xl">Audit Tenant</h1>
          </div>
          <p className="text-muted-foreground mt-1">
            {total.toLocaleString('id-ID')} kejadian tercatat di{' '}
      ...
      <form
        ...
        className="border-border bg-card grid grid-cols-1 gap-3 rounded-2xl border p-4 sm:grid-cols-5"
      >
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="contoh: tenant.webhook"
            className="border-border bg-background block w-full rounded-md border px-3 py-2 text-sm"
          />
        </div>
        <div className="space-y-1">
          <label htmlFor="f-actor" className="text-muted-foreground text-xs uppercase">
            Email aktor
          </label>
          <input
            id="f-actor"
        ...
        <div className="flex items-end gap-2 sm:col-span-5">
          ...
          {(validAction || resource || actorEmail || from || to) && (
            <Link
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
