...
import {
  parseAndValidateJobsCsv,
  bulkImportJobs,
  type PreviewResult,
  type ImportResult,
} from '@/lib/tenants/job-import-actions'
...
export function TenantJobsImportForm({ tenantSlug }: { tenantSlug: string }) {
  const router = useRouter()
  const { t } = useI18n()
  const tl = t.formsTenantImport.jobsImport
  ...
  function handlePreview() {
    if (!csvText.trim()) {
      setStage({ kind: 'upload', error: tl.csvEmptyError })
      return
    }
    startTransition(async () => {
      const res = await parseAndValidateJobsCsv({ tenantSlug, csvText })
      if (!res.ok) {
  ...
  function handleImport() {
    ...
    startTransition(async () => {
      const res = await bulkImportJobs({ tenantSlug, csvText: csv })
      if (!res.ok) {
        setStage({
          kind: 'preview',
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            disabled={pending}
            placeholder="title,location,description,employmentType,..."
    ...
    return (
      <div className="space-y-5">
        <div className="border-border flex flex-wrap items-center gap-4 rounded-2xl border bg-card p-4 text-sm">
          <div>
            <p className="text-muted-foreground text-xs uppercase">{tl.statTotalRows}</p>
          ...
          <div>
            <p className="text-muted-foreground text-xs uppercase">{tl.statInvalid}</p>
            <p className="text-destructive font-semibold">{preview.invalidCount}</p>
          </div>
        </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-muted/50 text-left">
              <tr>
                ...
                <th className="p-3 font-medium">{tl.colLocation}</th>
              ...
              {preview.rows.map((r) => {
                ...
                return (
                  <tr key={r.lineNum}>
                    ...
                    <td className="p-3 text-xs">
                      {(r.parsed?.location ?? r.raw.location ?? '').slice(0, 40)}
                    </td>
                    <td className="p-3 text-xs">
                      {isValid ? (
    ...
    <div className="space-y-5">
      ...
      >
        <div className="flex items-start gap-3">
          {allOk ? (
            <CheckCircle2 className="text-success mt-0.5 h-6 w-6" aria-hidden="true" />
          ) : (
            <AlertTriangle
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          ...
          href={`/dashboard/tenants/${tenantSlug}/jobs` as any}
          className={btnPrimary}
        >
          {tl.btnViewJobs}
        </Link>
