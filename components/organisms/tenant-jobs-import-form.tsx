...
export function TenantJobsImportForm({ tenantSlug }: { tenantSlug: string }) {
  ...
  function handlePreview() {
    if (!csvText.trim()) {
      setStage({ kind: 'upload', error: 'Tempel atau unggah CSV terlebih dahulu.' })
      return
    }
    ...
    return (
      <div className="space-y-5">
        <div className="border-border flex flex-wrap items-center gap-4 rounded-2xl border bg-card p-4 text-sm">
          <div>
            <p className="text-muted-foreground text-xs uppercase">Total baris</p>
          ...
          <div>
            <p className="text-muted-foreground text-xs uppercase">Bermasalah</p>
