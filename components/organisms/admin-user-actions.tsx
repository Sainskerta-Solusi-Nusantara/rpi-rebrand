...
export function AdminUserActions({
  ...
  function onRoleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (r.ok) {
        setBanner({ kind: 'success', message: 'Peran diperbarui.' })
        router.refresh()
      } else {
  ...
  function onStatusSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (r.ok) {
        setBanner({ kind: 'success', message: 'Status diperbarui.' })
        router.refresh()
      } else {
        setBanner({ kind: 'error', message: r.error })
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={onRoleSubmit} className="space-y-3">
        ...
        {!canChangeRole && (
          <p className="text-xs text-muted-foreground">
            Hanya SUPERADMIN yang dapat mengubah peran.
          </p>
        )}
        {isSelf && (
          <p className="text-xs text-muted-foreground">
      ...
      <form onSubmit={onStatusSubmit} className="space-y-3 border-t border-border pt-6">
        <label className="block text-sm font-medium text-foreground">Status akun</label>
        <select
          value={status}
