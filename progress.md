...
| 4   | `bcrypt` + `bcryptjs` terpasang bersamaan                          | low      | ✅ fixed (hapus bcryptjs)   |
...
| `2026-06-30 09:00` | `05-AUDIT`       | Hardening #4: konsolidasi bcrypt — seed pakai `bcrypt` (native), hapus `bcryptjs` dari deps. `npm run build` PASS (260 hal), tsc clean. Sisa audit terbuka: hanya #3 (FK, butuh keputusan arsitektur user).                                                                                                                                                          |
