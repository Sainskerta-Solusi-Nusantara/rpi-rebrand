...
const FROM_FALLBACK = 'no-reply@pekerja.sainskerta.net'
...
export function emailChangeVerifyEmail(opts: {
  ...
  const text = [
    greeting,
    '',
    `Anda meminta untuk mengganti email akun SSN dari ${opts.oldEmail} ke ${opts.newEmail}.`,
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:560px;margin:24px auto;color:#0f172a;line-height:1.6">
  <p>${greeting}</p>
  <p>Anda meminta untuk mengganti email akun SSN dari <strong>${opts.oldEmail}</strong> ke <strong>${opts.newEmail}</strong>. Klik tombol berikut untuk mengonfirmasi (berlaku 1 jam):</p>
...
export function emailChangeNoticeEmail(opts: {
  ...
  const greeting = opts.name ? `Halo ${opts.name},` : 'Halo,'
  const subject = 'Permintaan perubahan email akun SSN'
  const ipLine = opts.requestIp ? `IP permintaan: ${opts.requestIp}.` : ''
  const text = [
    greeting,
    '',
    `Kami menerima permintaan untuk mengganti email akun SSN Anda dari ${opts.oldEmail} ke ${opts.newEmail}.`,
    ipLine,
    '',
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:560px;margin:24px auto;color:#0f172a;line-height:1.6">
  <p>${greeting}</p>
  <p>Kami menerima permintaan untuk mengganti email akun SSN Anda dari <strong>${opts.oldEmail}</strong> ke <strong>${opts.newEmail}</strong>.</p>
...
export function loginAlertEmail(opts: {
  ...
  const greeting = opts.name ? `Halo ${opts.name},` : 'Halo,'
  const subject = 'Login baru terdeteksi di akun SSN Anda'
  const whenStr = opts.when.toISOString()
  const ipLine = opts.ip ? `${opts.ip} (sekitar ${opts.ipApprox})` : opts.ipApprox
  const text = [
    greeting,
    '',
    'Kami melihat login baru ke akun SSN Anda dari perangkat yang belum pernah dipakai sebelumnya:',
    '',
    `Waktu       : ${whenStr}`,
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:560px;margin:24px auto;color:#0f172a;line-height:1.6">
  <p>${greeting}</p>
  <p>Kami melihat login baru ke akun SSN Anda dari perangkat yang belum pernah dipakai sebelumnya:</p>
...
export function tenantInviteEmail(opts: {
  ...
  const inviter = opts.inviterName ?? 'Tim SSN'
  const text = [
    'Halo,',
    '',
    `${inviter} mengundang Anda bergabung ke ${opts.tenantName} di SSN Pekerja sebagai ${opts.role}.`,
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:560px;margin:24px auto;color:#0f172a;line-height:1.6">
  <p>Halo,</p>
  <p><strong>${inviter}</strong> mengundang Anda bergabung ke <strong>${opts.tenantName}</strong> di SSN Pekerja sebagai <strong>${opts.role}</strong>.</p>
...
export function emailVerificationEmail(opts: { name?: string | null; link: string }): {
  ...
  const greeting = opts.name ? `Halo ${opts.name},` : 'Halo,'
  const subject = 'Verifikasi alamat email akun SSN Anda'
  const text = [
    greeting,
    '',
    'Terima kasih telah mendaftar di SSN Pekerja.',
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:560px;margin:24px auto;color:#0f172a;line-height:1.6">
  <p>${greeting}</p>
  <p>Terima kasih telah mendaftar di SSN Pekerja. Klik tombol berikut untuk memverifikasi alamat email Anda (tautan berlaku 24 jam):</p>
...
export function passwordResetEmail(opts: { name?: string | null; link: string }): {
  ...
  const greeting = opts.name ? `Halo ${opts.name},` : 'Halo,'
  const subject = 'Reset password akun SSN Anda'
  const text = [
    greeting,
    '',
    'Kami menerima permintaan untuk mengatur ulang password akun SSN Anda.',
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:560px;margin:24px auto;color:#0f172a;line-height:1.6">
  <p>${greeting}</p>
  <p>Kami menerima permintaan untuk mengatur ulang password akun SSN Anda. Klik tombol berikut untuk melanjutkan (tautan berlaku 1 jam):</p>
...
export function applicationReceivedEmail(opts: {
  ...
  const text = [
    ...
    'Anda dapat memantau status lamaran kapan saja di dashboard SSN:',
    '',
    opts.applicationUrl,
    '',
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:560px;margin:24px auto;color:#0f172a;line-height:1.6">
  ...
  <p>Tim rekrutmen akan meninjau profil dan CV Anda dalam beberapa hari ke depan. Anda dapat memantau status lamaran kapan saja di dashboard SSN.</p>
...
export function applicationStatusEmail(opts: {
  ...
  const greeting = opts.name ? `Halo ${opts.name},` : 'Halo,'
  const footer = `— Tim ${opts.tenantName} (via SSN)`
  ...
  <p style="margin:24px 0"><a href="${opts.applicationUrl}" style="display:inline-block;background:hsl(220,50%,14%);color:#fff;padding:10px 18px;border-radius:6px;text-decoration:none;font-weight:600">Lihat status lamaran</a></p>`
    ...
    case 'SHORTLISTED': {
      ...
      bodyTextLines = [
        ...
        '',
        'Pastikan profil dan kontak Anda di SSN selalu terkini:',
        opts.applicationUrl,
      ]
      bodyHtml = `
  ...
  <p style="margin:24px 0"><a href="${opts.applicationUrl}" style="display:inline-block;background:hsl(220,50%,14%);color:#fff;padding:10px 18px;border-radius:6px;text-decoration:none;font-weight:600">Lihat status lamaran</a></p>`
    ...
    case 'INTERVIEW': {
      ...
      bodyTextLines = [
        ...
        '',
        'Detail jadwal wawancara (tanggal, waktu, tautan/lokasi) tersedia di dashboard SSN Anda. Mohon segera memeriksa dan mengonfirmasi ketersediaan Anda:',
        opts.applicationUrl,
        '',
  ...
  <p>Detail jadwal wawancara (tanggal, waktu, tautan/lokasi) tersedia di dashboard SSN Anda. Mohon segera memeriksa dan mengonfirmasi ketersediaan Anda.</p>
  ...
  <p style="font-size:13px;color:#475569">Tip: siapkan portofolio, dokumen pendukung, dan cek koneksi Anda jauh sebelum waktu wawancara.</p>`
    ...
    case 'OFFERED': {
      ...
      bodyTextLines = [
        ...
        '',
        'Detail penawaran (kompensasi, tanggal mulai, dan dokumen) tersedia di dashboard SSN Anda. Mohon tinjau dan respons sesegera mungkin:',
        opts.applicationUrl,
        '',
  ...
  <p>Detail penawaran (kompensasi, tanggal mulai, dan dokumen) tersedia di dashboard SSN Anda. Mohon tinjau dan respons sesegera mungkin.</p>
  ...
  <p style="font-size:13px;color:#475569">Sekali lagi, selamat — kami percaya Anda akan memberi kontribusi yang berarti.</p>`
    ...
    case 'REJECTED': {
      ...
      bodyTextLines = [
        ...
        '',
        'Kami sangat menghargai upaya, waktu, dan minat yang Anda tunjukkan selama proses ini. Profil Anda akan tetap tersimpan di SSN, dan kami sangat menganjurkan Anda untuk melamar peluang lain yang sesuai di masa depan — baik di kami maupun mitra SSN lainnya.',
        '',
  ...
  <p>Kami sangat menghargai upaya, waktu, dan minat yang Anda tunjukkan selama proses ini. Profil Anda akan tetap tersimpan di SSN, dan kami sangat menganjurkan Anda untuk melamar peluang lain yang sesuai di masa depan — baik di kami maupun mitra SSN lainnya.</p>
...
export function weeklyDigestEmail(opts: {
  ...
  const greeting = opts.name ? `Halo ${opts.name},` : 'Halo,'
  const subject = `Ringkasan mingguan akun SSN Anda (${opts.period})`
  const text = [
    greeting,
    '',
    `Berikut ringkasan aktivitas akun SSN Anda untuk periode ${opts.period}:`,
    '',
...
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:560px;margin:24px auto;color:#0f172a;line-height:1.6">
  <p>${greeting}</p>
  <p>Berikut ringkasan aktivitas akun SSN Anda untuk periode <strong>${opts.period}</strong>:</p>
  <table style="font-size:14px;border-collapse:collapse;margin:16px 0">
